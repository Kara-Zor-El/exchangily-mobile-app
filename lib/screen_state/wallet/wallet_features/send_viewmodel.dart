...
----------------------------------------------------------------------*/
  checkAmount() async {
    ...
    log.e(amount);
    var res = RegexValidator(pattern as String).isValid(amount.toString());
    ...
    if (res) {
      if (!isTrx()) {
        ...
        trxBalance = await (getTrxBalance() as FutureOr<double>);
  ...
  Future<double?> getTrxBalance() async {
    ...
    String? trxWalletAddress = await walletService!
